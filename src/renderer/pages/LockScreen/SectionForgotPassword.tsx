...
export const SectionForgotPassword: FC<Props> = ({ email, onClose, onVerificationCode, onSuccess, onGoBack }): ReactElement => {
  const inputCodeRef = []
  ...
  const checkVerificationCode = async (code) => {
    if (code.length == VERIFY_CODE_LENGTH) {
      ...
      ipcRenderer.send(ApiCommand.ResetWithVerificationCode, { email, code, guestUsername: guestUser?.email })
    }
  }
    ...
    ipcRenderer.on(ApiMessage.LogInResult, async (event, logInResult) => {
      if (logInResult.success) {
        ...
        setTimeout(() => {
          ...
          onSuccess(logInResult.user.email)
        }, 500)
      } else {
