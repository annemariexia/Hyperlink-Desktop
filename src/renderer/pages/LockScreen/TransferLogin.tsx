...
import { SectionTransferWelcome } from "./SectionTransferWelcome"
...
const FADE_BACKGROUND_DELAY_MS = 5000
const FADE_BACKGROUND_DURATION_MS = 1000

export const TransferLogin: FC<Props> = ({ setProfile, setDevices, onClose, profile, setIsShowTutorial }): ReactElement => {
  ...
  return (
    <>
      <ModalOverlay fade={modalStyle} onClick={close}>
        <Background showBackgroundFade={showBackgroundFade} />
        <Container data-testid="screen-login">
          {step === Step.Welcome && (
            <SectionTransferWelcome
              email={email}
              photoUrl={photoUrl}
              userInfo={userInfo}
...
const Background = styled.div<{ showBackgroundFade: boolean }>`
  ...
  transition: opacity ${FADE_BACKGROUND_DURATION_MS}ms ease-in-out;
  z-index: 999;
`
