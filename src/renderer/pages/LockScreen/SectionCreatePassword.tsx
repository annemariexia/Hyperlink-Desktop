...
import imgSignup from "./../../../../images/HeartResetPass.png"
...
export const getWelcomeTitle = (userInfo: UserBasicInfo | null) => {
  if (!userInfo) return "Reset your password"
  if (!!userInfo?.firstName && userInfo?.firstName.length > 0) return `Reset your password, ${userInfo.firstName}!`
  return "Reset your password"
}

export const SectionCreatePassword: FC<Props> = ({ email, userInfo, onSuccess, onOpenForgotPassword, onVerificationCode, onClose, onGoBack }): ReactElement => {
  ...
  //function for password Show/Hide button
  const toggleShowPassword = () => {
    setShowPassword((prevState) => !prevState)
  }

  //function for conform password Show/Hide button
  const toggleShowConformPassword = () => {
    setShowConformPassword((prevState) => !prevState)
  }
  ...
  const reset = async (email: string, password: string) => {
    ...
    ipcRenderer.send(ApiCommand.ResetPassword, { email, password, guestUsername })
  }
  ...
  useEffect(() => {
    ipcRenderer.on(ApiMessage.ResetPasswordResult, async (event, logInResult) => {
    ...
    return () => {
      ...
      ipcRenderer.removeAllListeners(ApiMessage.ResetPasswordResult)
    }
  }, [])
  ...
  const isActionButtonDisabled = !password || password.length === 0 || !confirmPassword || confirmPassword.length === 0
  ...
  return (
    <Form onClick={stopEventPropagation}>
      <div className="leftPanel">
        ...
        <div className="bottomText" style={{ fontSize: "28px", lineHeight: "28px", letterSpacing: "-0.7px" }}>
          Get amazing <br />
          rewards with <br />
          your earnings
        </div>
      ...
      <div className="rightPanel">
        ...
        <Content>
          <Name>{title}</Name>
          <Description>Enter your new password. Create a strong password with a mix of letters, numbers and symbols.</Description>
          <PasswordTitle>Create a new password</PasswordTitle>
          <PasswordInputContainer error={!!error}>
            <PasswordInput name="password" type={showPassword ? "text" : "password"} placeholder="Password" value={password} onInput={onPasswordChanged} autoFocus />
          ...
          <PasswordTitle style={{ marginTop: "24px" }}>Confirm</PasswordTitle>
          <PasswordInputContainer error={!!confirmError}>
            <PasswordInput name="confirm_password" type={showConformPassword ? "text" : "password"} placeholder="Password" value={confirmPassword} onInput={onConfirmPasswordChanged} />
            {confirmPassword != "" && <ShowHideButton onClick={toggleShowConformPassword}> {showConformPassword ? "Hide" : "Show"}</ShowHideButton>}
          </PasswordInputContainer>
          {confirmError && <ErrorDescription>{confirmError}</ErrorDescription>}
          <ContinueButton errorCompare={confirm} onClick={isActionButtonDisabled ? undefined : onSubmit} disabled={isActionButtonDisabled}>
            {isLoading ? <DotFlashing /> : <ContinueButtonLabel> Reset password </ContinueButtonLabel>}
          </ContinueButton>
        </Content>
      </div>
    </Form>
...
const ContinueButton = styled.div<{ disabled: boolean; errorCompare: boolean }>`
  display: flex;
  padding: 12px 12px;
  flex-direction: column;
  align-items: center;
